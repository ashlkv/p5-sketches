...
new p5((p5) => {
    ...
    p5.draw = () => {
        ...
        cells.forEach((cell, index) => {
            ...
            const rate = flowField.getValueAt(center);
            p5.push()
            ...
            if ((up && rate > 0) || (!up && rate < 0)) {
                ...
                if (visibleSides.length > 0) {
                    const relativeSides = visibleSides.map(line => moveVertices(line, {x: -center.x, y: -center.y}))
                    relativeSides.forEach(([{x: x1, y: y1}, {x: x2, y: y2}]) => {
                        p5.line(x1, y1, x2, y2)
                    })
                }*/
                const relative = moveVertices(cell, {x: -center.x, y: -center.y})
                p5.beginShape()
